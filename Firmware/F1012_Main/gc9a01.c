...
const uint32 gc9a01_color_white = 0xFFFF;
const uint32 gc9a01_color_black = 0x0000;
const uint32 gc9a01_color_red = 0xF800;
const uint32 gc9a01_color_green = 0x07E0;
const uint32 gc9a01_color_blue = 0x001F;
const uint32 gc9a01_color_cyan = 0x07FF;
const uint32 gc9a01_color_orange = 0xFFE0;
const uint32 gc9a01_color_purple = 0xF81F;
...
void gc9a01_point(uint16_t x, uint16_t y, uint32_t color){
	gc9a01SetDrawWindow(x, y, x+1, y+1);
...
void gc9a01_vert_line(uint16_t x, uint16_t y0, uint16_t y1, uint32_t color){
	gc9a01SetDrawWindow(x, y0, x+1, y1);
...
void gc9a01Init(void){
	...
	gc9a01_send_single_cmd_data(0xEB, 0x14);

	_gc9a01_send_command(DISP_CMD_INTER_REG_EN1); _gc9a01_send_command(DISP_CMD_INTER_REG_EN2);
	gc9a01_send_single_cmd_data(0xEB, 0x14);

	gc9a01_send_single_cmd_data(0x84, 0x40);
	gc9a01_send_single_cmd_data(0x85, 0xFF);

	gc9a01_send_single_cmd_data(0x86, 0xFF);
	gc9a01_send_single_cmd_data(0x87, 0xFF);
	gc9a01_send_single_cmd_data(0x88, 0x0A);
	gc9a01_send_single_cmd_data(0x89, 0x21);
	gc9a01_send_single_cmd_data(0x8A, 0x00);
	gc9a01_send_single_cmd_data(0x8B, 0x80);
	gc9a01_send_single_cmd_data(0x8C, 0x01);
	gc9a01_send_single_cmd_data(0x8D, 0x01);
	gc9a01_send_single_cmd_data(0x8E, 0xFF);
	gc9a01_send_single_cmd_data(0x8F, 0xFF);

	_gc9a01_send_cmd_and_data(0x90, (uint8_t[]){0x08, 0x08, 0x08, 0x08}, 4);

	gc9a01_send_single_cmd_data(0xBD, 0x06);

	gc9a01_send_single_cmd_data(0xBC, 0x00);

	_gc9a01_send_cmd_and_data(0xFF, (uint8_t[]){0x60, 0x01, 0x04}, 3);

	gc9a01_send_single_cmd_data(0xC3, 0x13);
	gc9a01_send_single_cmd_data(0xC4, 0x13);

	gc9a01_send_single_cmd_data(0xC9, 0x22);

	gc9a01_send_single_cmd_data(0xBE, 0x11);
    ...
    gc9a01_send_single_cmd_data(0xAE, 0x77);

    gc9a01_send_single_cmd_data(0xCD, 0x63);

    _gc9a01_send_cmd_and_data(0x70, (uint8_t[]){0x07, 0x07, 0x04, 0x0E, 0x0F, 0x09, 0x07, 0x08, 0x03}, 9);
    gc9a01_send_single_cmd_data(0xE8, 0x34);
	...
	gc9a01_send_single_cmd_data(DISP_CMD_PIXEL_FORMAT_SET, 0x05);
